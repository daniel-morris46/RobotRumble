...
public class InGameMenuPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (currentHexes != null) { // If the current list of hexes is not null
            ...
            for (int i = 0; i < currentHexes.length; i++) { // For each hex row of hexes
                ...
                for (int j = 0; j < currentHexes[i].length; j++) { // For each hex in the current
                                                                   // row

                    if (currentHexes[i][j] != null) // If hex[i][j] exists
                                                    // Draw the hex and add the board size to
    ...
    private void drawTeamPanel(RobotTeam currentTeam) {
        ...
        for (int i = 0; i < 3; i++) { // For each robot, make a label and add robot stats
            ...
            statString = "<html> Movement: " + currentRobot.getMovementCur() + "/"
                    + currentRobot.getMovementMax() + "<br>";
    ...
    private void drawRobot(Robot toDraw, Graphics2D g) {
        ...
        int y = (robotHex.getPositionY() + boardSize) * (s + t); // Accounts for the board
                                                                 // coordinate offset and centers Y
                                                                 // Accounts for the board
                                                                 // coordinate offset and centers X
        int x = (robotHex.getPositionX() + boardSize) * h
                + (robotHex.getPositionY() + boardSize) * (h / 2);

        g.drawImage(getRobotImage(toDraw), x + r / 2, y + s / 2, this); // Draws the robot
    }

    /**
     * Creates a hexagonal polygon at the given x and y positions
     * 
    ...
    /**
     * Loads the correct image file from the resource folder based on the type of robot
     * 
